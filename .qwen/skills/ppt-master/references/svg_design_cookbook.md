...
> 精确的 X 坐标在后续处理阶段会由 `svg_auto_repair.py` 脚本自动校验和修正。
...
> [!IMPORTANT]
> **同布局内多样化（Intra-Layout Diversity）**
>
> 布局类型由 Design Spec 决定，你不能违反。但 **同一种布局类型的不同页面，内部视觉表现必须有差异**。
>
> **问题本质**：在并行 batch 生成中，每个 batch 的 AI 独立工作，看不到其他 batch 的输出。
> 所有 AI 都倾向于用最"安全"、最熟悉的内部排版方式——结果最终拼合时，10 个三栏页长得几乎一模一样。
>
> **核心原则**：始终假设你这一批之外，已经有很多页使用了和你相同的布局类型，并且它们都用了最常见的那种内部排版。你必须做出区别。
>
> **同为三栏卡片，至少在以下维度中变换 2 项**：
>
> | 维度 | 常见做法（已被大量使用） | 你应该尝试的变化 |
> |------|----------------------|----------------|
> | 标题栏 | 纯色 55px 标题栏 | 渐变标题栏 / 更高的 70px 标题栏 / 加副标题 |
> | 数据展示 | 大数字 + 单位文字 | 嵌套白卡指标块(C10) / 并排 Badge 行(C9) / 数据进度条 |
> | 列表样式 | `•` 前缀列表 | 信息卡(C8)图标+标题+说明 / 编号列表 / 带图标的条目 |
> | 底部区域 | 分割线 + 一行底注 | 浅色底总结框 / 底部指标行(L4) / Badge 标签行 |
> | 装饰手法 | 左侧色条 6×36px | 顶部全宽色条 / 角标数字圆 / 浅色背景区块 |
>
> **简言之**：不要每次三栏都写成 `标题栏 → 大数字 → Badge → 列表 → 底注` 这一种套路。
> cookbook 里的 C8(信息卡)、C9(并排Badge)、C10(嵌套白卡) 就是为了给你提供内部变化的武器。
