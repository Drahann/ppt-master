...
def process_markdown_images(markdown: str, job_dir: Path) -> tuple[str, list[str]]:
    ...
    return IMAGE_PATTERN.sub(replace, markdown), warnings
...
def _download_remote_image(url: str, alt: str, assets_dir: Path, index: int) -> str | None:
    ...
    suffix = MIME_TO_SUFFIX.get(content_type) or Path(unquote(urlparse(url).path)).suffix or mimetypes.guess_extension(content_type) or ".bin"
    if suffix == ".jpe":
        suffix = ".jpg"
    target = _unique_target(assets_dir, _safe_stem(alt), suffix, index)
...
def _copy_local_image(path_like: str, alt: str, assets_dir: Path, index: int) -> str | None:
    ...
    if resolved is None or not resolved.exists() or not resolved.is_file():
        return None
    target = _unique_target(assets_dir, _safe_stem(alt), resolved.suffix or ".bin", index)
