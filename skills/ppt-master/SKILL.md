...
Read references/svg_design_cookbook.md    # REQUIRED: SVG visual design cookbook
...
> Only need to read executor-base + svg_design_cookbook + one style file.
> If a runtime anchor file exists (for example `runner/svg_anchor_context.json`), read it before the first SVG page and re-read it together with the cookbook at each long-run re-anchor checkpoint.
...
| SVG design cookbook | `references/svg_design_cookbook.md` |
