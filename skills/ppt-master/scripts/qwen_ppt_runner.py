...
def build_svg_review_input(
    project_path: Path,
    plan: list[SlidePlanEntry],
    valid_chart_keys: set[str],
    runner_dir: Path,
) -> dict[str, Any]:
    state_complete, deterministic_issues = check_generation_state(
        project_path,
        plan,
        valid_chart_keys,
        runner_dir,
    ...
    actual_svg_names = sorted(path.name for path in svg_dir.glob("*.svg"))
    return {
        ...
        "svg_quality_report_path": str(runner_dir / SVG_QUALITY_REPORT_FILENAME),
        "expected_svg_names": [entry.filename for entry in plan],
        "actual_svg_names": actual_svg_names,
...
def check_svg_review_state(
    ...
    errors: list[str] = []
    generation_complete, generation_errors = check_generation_state(
        project_path,
        plan,
        valid_chart_keys,
        runner_dir,
...
def build_svg_review_bootstrap_prompt(
    project_path: Path,
...
13. {svg_anchor_context_path}
14. {project_path / "notes" / "total.md"}
...
def build_svg_review_continue_prompt(
    ...
    bullet_errors = "\n".join(f"- {item}" for item in generation_errors)
    return f"""Continue the SVG repair review gate.

Project path: {project_path}
The SVG outputs are still failing these checks:
{bullet_errors}
